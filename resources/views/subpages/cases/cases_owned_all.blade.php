...
<div class="col-md-12 row m-0 p-0 align-items-center">
    ...
    <div class="col-12 row text-right justify-content-end">
        <div class="mb-2 mr-auto">
            <a class="btn btn-primary btn-lg" href="{{route('cases')}}">
                {{__('buttons.active')}}
            </a>
            @if(!$filter_is_archived)
            <a class="btn btn-primary btn-lg" href="{{route('cases.filters.archive',['is_archived'=>1])}}">
                {{__('buttons.archive')}}
            </a>
            @endif
        </div>
    ...
    <form id="form_item_search_status" action="" method="POST" class="hidden" onsubmit="FilterCases(event, this, '/cases')">
        @csrf
        <input type="hidden" name="filters" value="{{$filters ?? ''}}">
    </form>
    <form id="form_item_search_name" action="" method="POST" class="hidden" onsubmit="FilterCases(event, this, '/cases')">
        @csrf
        <input type="hidden" name="filters" value="{{$filters ?? ''}}">
    </form>
    <form id="form_item_search_motion" action="" method="POST" class="hidden" onsubmit="FilterCases(event, this, '/cases')">
        @csrf
        <input type="hidden" name="filters" value="{{$filters ?? ''}}">
    </form>
    <form id="form_item_search_petitioner" action="" method="POST" class="hidden" onsubmit="FilterCases(event, this, '/cases')">
        @csrf
        <input type="hidden" name="filters" value="{{$filters ?? ''}}">
    </form>
    <form id="form_item_search_lawyer" action="" method="POST" class="hidden" onsubmit="FilterCases(event, this, '/cases')">
        @csrf
        <input type="hidden" name="filters" value="{{$filters ?? ''}}">
    </form>
</div>
