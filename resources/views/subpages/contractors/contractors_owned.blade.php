        ...
        <table class="table table-sm">
            ...
            <tbody>
                ...
                <tr class="@if($model->id==$current_id) background-table-body-dark @endif">
                    ...
                    <td >{{$model->shortname}} @if($model->type_id==1) {{$model->fullname}} @endif</td>
                    <td >{{$model->nip}}</td>
                    <td >{{$model->postal_code}} {{$model->city}} {{$model->street}}</td>
                    <td >
                        <a class="collapse-arrow-toggle collapse-trigger btn btn-primary btn-lg"  href="{{route('contractors.expanded',['id'=>$model->id])}}">
                            <i class="fas fa-eye"></i>
                        </a>
                        @if($model->id==$current_id)
                        <a class="collapse-arrow-toggle collapse-trigger btn btn-primary btn-lg"  href="{{route('contractors')}}">
